...
public class BoardController {
    ...
    public ResponseEntity<Object> insertBoard(@RequestBody BoardDto boardDto, Authentication authentication)
            throws Exception {
        UserDto userDto  = (UserDto) aturhentication.getPrincipal();
    ...
    public ResponseEntity<Map<String,Object>> boardDetail(@PathVatiable("b_idx") int bIdx, @PathVariable("update") int update) throws Exception{
    ...
    public ResponseEntity<Object> insertComments(@RequestBody BoardCommentsDto boardCommentsDto, Authentication authentication){
        ...
        boardCommentsDto.setUserId(userDto.getUserId()); //안 나오면 이건  userId가 아니라 user_nickname  or userNickname 으로 바꿔야
